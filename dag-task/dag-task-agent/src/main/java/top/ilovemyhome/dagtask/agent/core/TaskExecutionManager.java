...
public class TaskExecutionManager {
    ...
    public TaskExecutionManager(AgentConfiguration config, DagServerClient dagServerClient,
    ...
    public ForceOkResult forceOk(Long taskId) {
        ...
        if (removedPending) {
            ...
            dagServerClient.reportResult(taskId, true, "{\"forced\":true}");
    ...
    private void executeTask(PendingTask pendingTask) {
        ...
        try {
            ...
            dagServerClient.reportResult(taskId, output.isSuccess(), serializeOutput(output));
            finishTask(taskId, true, output.isSuccess(), duration);
        } catch (Exception e) {
            long duration = System.currentTimeMillis() - startTime;
            LOGGER.error("Task {} execution failed after {}ms", taskId, duration, e);
            dagServerClient.reportResult(taskId, false, e.getMessage());
