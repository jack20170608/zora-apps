...
public class TaskRecordDaoJdbiImpl extends BaseDaoJdbiImpl<TaskRecord> implements TaskRecordDao, TaskRecordRepository {
    ...
    /** New port method using Instant (TaskRecordRepository). */
    @Override
    public int start(Long id, TaskInput input, Instant startDt) {
        Objects.requireNonNull(startDt);
        return start(id, input, LocalDateTime.ofInstant(startDt, ZoneId.systemDefault()));
    }

    /** Legacy method using LocalDateTime (TaskRecordDao). Kept for binary compatibility. */
    @Override
    ...
    /** New port method using Instant (TaskRecordRepository). */
    @Override
    public int stop(Long id, TaskStatus newStatus, TaskOutput output, Instant endDt) {
        Objects.requireNonNull(endDt);
        return stop(id, newStatus, output, LocalDateTime.ofInstant(endDt, ZoneId.systemDefault()));
    }
    ...
    public Page<TaskRecord> find(TaskRecordSearchCriteria criteria, Pageable pageable) {
        ...
        top.ilovemyhome.zora.jdbi.page.Page<TaskRecord> zoraPage = super.find(criteria, zoraPageable);
